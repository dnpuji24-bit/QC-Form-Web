...
function newFilling(index = 1): Filling {
  return { id: uid(), pengisianKe: index, jumlah: '', hasilKerja: '', pemerataanPupuk: '' }
}
function newCard(): UnitCard {
  return {
    id: uid(), unit: '', noUnit: '', paddock: '', type: '', activity: '', jenisPupuk: '', dosis: '', statusHose: 'Lancar', catatan: '',
    fillings: [newFilling(1)], downtime: [],
  }
}
...
export default function FertilizerForm({ token, user, master, onSaved }: Props) {
  ...
  function patchFilling(cardId: string, fillingId: string, patch: Partial<Filling>) {
    setCards((old) => old.map((card) => card.id === cardId ? {
      ...card, fillings: card.fillings.map((f) => f.id === fillingId ? { ...f, ...patch } : f),
    } : card))
  }
  function addFilling(cardId: string) {
    setCards((old) => old.map((card) => card.id === cardId ? { ...card, fillings: [...card.fillings, newFilling(card.fillings.length + 1)] } : card))
  }
  function removeFilling(cardId: string, fillingId: string) {
    setCards((old) => old.map((card) => card.id === cardId ? {
      ...card,
      fillings: card.fillings.filter((f) => f.id !== fillingId).map((f, i) => ({ ...f, pengisianKe: i + 1 })),
    } : card))
  }
  ...
  function validate() {
    ...
    for (let i = 0; i < cards.length; i++) {
      const card = cards[i]
      if (!card.unit || !card.noUnit || !card.paddock || !card.activity || !card.jenisPupuk || num(card.dosis) <= 0) return `Unit Card ${i + 1}: lengkapi Unit, No. Unit, Paddock, Activity, Jenis Pupuk, dan Dosis Target.`
      ...
      for (const fill of card.fillings) if (num(fill.jumlah) <= 0 || num(fill.hasilKerja) <= 0) return `Unit ${card.noUnit}: Jumlah pupuk dan Hasil Kerja pada setiap pengisian wajib lebih dari 0.`
    }
    return ''
  }
    ...
    try {
      ...
      for (const card of cards) {
        ...
        const record: QcRecord = {
          ...
          type: card.type || 'Fertilizer', activity: card.activity, jenisPupuk: card.jenisPupuk,
          dosis: num(card.dosis), statusHose: card.statusHose, pengisianList, downtimeList: card.downtime,
          catatan, noted: catatan, photoBase64: await photoData(card.photo), saveType,
          inputtedBy: user.username, createdAt: new Date().toISOString(),
        }
  ...
  return <section>
    <div className="section-head">
      <div><div className="eyebrow">FERTILIZER DAILY SESSION</div><h2>Input Fertilizer per Unit</h2></div>
      <span className="badge">Unit-centric</span>
    </div>
    ...
    <div className="mode-switch panel">
      <button className="primary" type="button">+ Input Manual</button>
      <button type="button" disabled title="Disiapkan untuk tahap OCR tulisan tangan berikutnya">📷 Foto Laporan — segera</button>
      <span className="muted">Struktur Unit Card ini sudah disiapkan agar hasil pembacaan foto nantinya masuk ke field yang sama.</span>
    </div>
      ...
      {cards.map((card, index) => {
        ...
        return <article className="unit-card" key={card.id}>
          ...
          <div className="form-grid">
            ...
            <label>Jenis Pupuk<input list={`fert-${card.id}`} value={card.jenisPupuk} onChange={(e) => { const value = e.target.value; const mat = (master.materials || []).find((m: MaterialMaster) => String(m.material) === value); patchCard(card.id, { jenisPupuk: value, dosis: card.dosis || String(mat?.dosage || '') }) }} /><datalist id={`fert-${card.id}`}>{fertilizerNames.map((x) => <option value={x} key={x} />)}</datalist></label>
            <label>Dosis Target (Kg/Ha)<input type="number" step=".01" value={card.dosis} onChange={(e) => patchCard(card.id, { dosis: e.target.value })} /></label>
            <label>Status Hose<select value={card.statusHose} onChange={(e) => patchCard(card.id, { statusHose: e.target.value })}><option>Lancar</option><option>Tidak Lancar</option></select></label>
          ...
          <section className="subpanel">
            ...
            <div className="filling-list">
              {card.fillings.map((f) => {
                ...
                return <div className="filling-row" key={f.id}>
          ...
          <section className="subpanel">
            ...
            {card.downtime.map((d) => <div className="downtime-row" key={d.id}>
              <input placeholder="Issue" value={d.issue} onChange={(e) => patchDowntime(card.id, d.id, { issue: e.target.value })} />
              <input type="time" value={d.start} onChange={(e) => patchDowntime(card.id, d.id, { start: e.target.value })} />
              <input type="time" value={d.end} onChange={(e) => patchDowntime(card.id, d.id, { end: e.target.value })} />
              <input placeholder="Catatan" value={d.note} onChange={(e) => patchDowntime(card.id, d.id, { note: e.target.value })} />
              <button type="button" className="danger" onClick={() => patchCard(card.id, { downtime: card.downtime.filter((x) => x.id !== d.id) })}>×</button>
            </div>)}
          </section>

          <div className="form-grid">
            <label className="span-2">Catatan<textarea rows={3} value={card.catatan} onChange={(e) => patchCard(card.id, { catatan: e.target.value })} /></label>
            <label>Foto QC<input type="file" accept="image/*" capture="environment" onChange={(e) => patchCard(card.id, { photo: e.target.files?.[0] })} /></label>
          </div>
    ...
    <div className="panel session-summary">
      <div><span>Unit aktif</span><strong>{total.units}</strong></div>
      <div><span>Total pupuk</span><strong>{total.totalKg.toLocaleString('id-ID')} Kg</strong></div>
      <div><span>Total hasil</span><strong>{total.totalHa.toLocaleString('id-ID')} Ha</strong></div>
    </div>
