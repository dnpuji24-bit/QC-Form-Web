...
type Downtime={id:string;issue:string;start:string;end:string;note:string;file?:File;photoDriveUrl?:string}
type UnitCard={id:string;recordId?:string;createdAt?:string;photoDriveUrl?:string;saveType?:string;unit:string;noUnit:string;paddock:string;type:string;activity:string;catatan:string;photo?:File;fillings:Filling[];downtime:Downtime[]}
...
async function photoData(file?:File):Promise<string>{if(!file)return'';if(file.size>6_000_000)throw new Error('Foto asli maksimal 6 MB.');let image:CanvasImageSource,width=0,height=0;if('createImageBitmap'in window){const bitmap=await createImageBitmap(file);image=bitmap;width=bitmap.width;height=bitmap.height}else{const url=URL.createObjectURL(file);const img=await new Promise<HTMLImageElement>((resolve,reject)=>{const x=new Image();x.onload=()=>resolve(x);x.onerror=reject;x.src=url});image=img;width=img.naturalWidth;height=img.naturalHeight;URL.revokeObjectURL(url)}const scale=Math.min(1,1400/width),canvas=document.createElement('canvas');canvas.width=Math.round(width*scale);canvas.height=Math.round(height*scale);const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Foto tidak dapat diproses.');ctx.drawImage(image,0,0,canvas.width,canvas.height);let quality=.72,result=canvas.toDataURL('image/jpeg',quality);while(result.length>2_400_000&&quality>.42){quality-=.08;result=canvas.toDataURL('image/jpeg',quality)}if(result.length>2_700_000)throw new Error('Foto masih terlalu besar setelah kompresi.');return result}

export default function FertilizerForm({token,user,master,initialRecords=[],onCancelEdit,onSaved}:Props){
  const firstInitial=initialRecords[0],editing=initialRecords.length>0,editingUploaded=initialRecords.some(r=>r.saveType==='uploaded'),draftKey=`fertilizer:${user.username}`
  ...
  useEffect(()=>{if(editing){setDraftReady(true);return}let alive=true;void loadDraft<FertDraft>(draftKey).then(saved=>{if(!alive||!saved)return;setSessionId(saved.sessionId||sessionId);setDate(saved.date||today());setShift(saved.shift||'1');setMandor(saved.mandor||'');setAssistant(saved.assistant||'');if(saved.cards?.length){setCards(saved.cards);setActiveCardId(saved.activeCardId||saved.cards[0].id)}}).finally(()=>{if(alive)setDraftReady(true)});return()=>{alive=false}},[draftKey,editing])
  useEffect(()=>{if(!editing&&draftReady)void saveDraft<FertDraft>(draftKey,{sessionId,date,shift,mandor,assistant,cards,activeCardId})},[draftKey,editing,draftReady,sessionId,date,shift,mandor,assistant,cards,activeCardId])
  ...
  function duplicateCard(card:UnitCard){const copy:UnitCard={...card,id:uid(),recordId:undefined,createdAt:undefined,saveType:undefined,noUnit:'',photo:undefined,photoDriveUrl:undefined,fillings:card.fillings.map((f,i)=>({...f,id:uid(),pengisianKe:i+1,jumlah:'',hasilKerja:'',pemerataanPupuk:''})),downtime:[],catatan:''};setCards(old=>[...old,copy]);setActiveCardId(copy.id)}
