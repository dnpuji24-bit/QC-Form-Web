...
export default function PlanSummaryDashboard(){
  ...
  return <section className="plan-summary-dashboard">
    ...
    <section className="panel summary-day-matrix-panel">
      ...
      <div className="summary-wide-table"><table className="summary-plan-report-table"><thead><tr><th rowSpan={2} className="summary-sticky-col first">Kegiatan / Activity</th>{dayMatrixDays.map(date=><th key={date} colSpan={2}>{dayLabel(date)}</th>)}</tr><tr>{dayMatrixDays.map(date=><Fragment key={date}><th>Plan</th><th>Report</th></Fragment>)}</tr></thead><tbody>{dayMatrixRows.map(row=><tr key={row.activity}><td className="summary-sticky-col first"><strong>{row.activity}</strong></td>{row.days.map(cell=><Fragment key={row.activity+'|'+cell.date}><td className={cell.plan>0?'summary-plan-cell':''}>{cell.plan>0?new Intl.NumberFormat('id-ID',{maximumFractionDigits:2}).format(cell.plan):'-'}</td><td className={cell.report<=0&&cell.plan>0?'summary-report-low':cell.report>0&&cell.plan>0&&cell.report>=cell.plan?'summary-report-good':cell.report>0?'summary-report-only':''}>{cell.report>0?new Intl.NumberFormat('id-ID',{maximumFractionDigits:2}).format(cell.report):cell.plan>0?'0':'-'}</td></Fragment>)}</tr>)}</tbody></table></div>
