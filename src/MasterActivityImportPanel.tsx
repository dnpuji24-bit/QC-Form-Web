...
type ParsedActivity={docId:string;activityCode:string;description:string;activity:string;type:string;activityCategory:string;companyScope:string;defaultUnitName:string;defaultShift:string;defaultForeman:string;active:boolean;components:ActivityComponent[];componentCount:number}
...
type ActivityBase={description:string;activity:string;type:string;activityCategory:string;defaultUnitName:string;defaultShift:string;defaultForeman:string;sourceRow:number;occurrence:number}
...
function activityDocId(scope:string,description:string,occurrence=1){const base=`${slug(scope)}__${slug(description)}`;return occurrence>1?`${base}__dup-${occurrence}`:base}
function materialDocId(name:string,occurrence=1){const base=slug(name);return occurrence>1?`${base}__dup-${occurrence}`:base}
...
function reviewComponentKey(component:ActivityComponent){return`${normalized(component.activeIngredient)}|${Number(component.dosePerHa).toFixed(8)}|${normalized(component.unit)}`}
function reviewRecipeSignature(components:ActivityComponent[]){return components.map(reviewComponentKey).sort().join('||')}
function componentsFromData(value:unknown):ActivityComponent[]{if(!Array.isArray(value))return[];return value.map((item,index)=>{const row=(item&&typeof item==='object'?item:{}) as Record<string,unknown>;return{sequence:Number(row.sequence)||index+1,label:text(row.label)||`Bahan ${index+1}`,activeIngredient:text(row.activeIngredient),dosePerHa:Number(row.dosePerHa)||0,unit:text(row.unit)}}).filter(item=>item.activeIngredient&&item.dosePerHa>0&&item.unit)}
...
async function countPostImportReviewIssues(){
  if(!firestoreDb)return 0
  const[activitySnap,materialSnap]=await Promise.all([getDocs(collection(firestoreDb,'master_activities')),getDocs(collection(firestoreDb,'master_materials'))])
  let count=0
  const descriptionMap=new Map<string,number>();activitySnap.docs.forEach(item=>{const data=item.data() as Record<string,unknown>,key=normalized(text(data.description));if(key)descriptionMap.set(key,(descriptionMap.get(key)||0)+1)});descriptionMap.forEach(total=>{if(total>1)count++})
  const materialMap=new Map<string,number>();materialSnap.docs.forEach(item=>{const data=item.data() as Record<string,unknown>,key=normalized(text(data.materialName));if(key)materialMap.set(key,(materialMap.get(key)||0)+1)});materialMap.forEach(total=>{if(total>1)count++})
  const recipeMap=new Map<string,number>()
  activitySnap.docs.forEach(item=>{const data=item.data() as Record<string,unknown>,components=componentsFromData(data.components);if(!components.length)return;const signature=reviewRecipeSignature(components);recipeMap.set(signature,(recipeMap.get(signature)||0)+1);const own=new Set<string>();let duplicateInside=false;components.forEach(component=>{const key=reviewComponentKey(component);if(own.has(key))duplicateInside=true;own.add(key)});if(duplicateInside)count++})
  recipeMap.forEach(total=>{if(total>1)count++})
  return count
}
...
function parseWorkbook(buffer:ArrayBuffer,fileName:string,scope:string,codes:ExistingCodeMap):ParsedFile{
  ...
  const issues:Issue[]=[],activityBases:ActivityBase[]=[],knownDescriptions=new Set<string>(),descriptionOccurrences=new Map<string,number>()

  validationRows.forEach((row,index)=>{
    const description=text(rowValue(row,'Deskripsi')),activity=text(rowValue(row,'Activity')),type=normalizeType(text(rowValue(row,'Type'))),activityCategory=normalizeCategory(text(rowValue(row,'Activity Category'))),defaultUnitName=text(rowValue(row,'Default Unit','Unit','Kode / Nama Unit','Nama Unit')),defaultShift=text(rowValue(row,'Default Shift','Shift')),defaultForeman=text(rowValue(row,'Default Mandor','Mandor','Foreman','Nama Mandor','Nama Foreman')),sourceRow=index+2
    ...
    const key=normalized(description),occurrence=(descriptionOccurrences.get(key)||0)+1;descriptionOccurrences.set(key,occurrence);knownDescriptions.add(key)
    if(occurrence>1)issues.push({level:'WARNING',item:description,message:`Duplikat Deskripsi pada ${VALIDATION_SHEET}: ${description}. Import tetap diizinkan sebagai record terpisah untuk Review Pasca Import.`})
    activityBases.push({description,activity,type,activityCategory,defaultUnitName,defaultShift,defaultForeman,sourceRow,occurrence})
  })
  ...
  const materials:ParsedMaterial[]=[],materialOccurrences=new Map<string,number>(),materialByIngredient=new Map<string,ParsedMaterial[]>()
  materialRows.forEach((row,index)=>{
    ...
    const key=normalized(materialName),occurrence=(materialOccurrences.get(key)||0)+1;materialOccurrences.set(key,occurrence)
    if(occurrence>1)issues.push({level:'WARNING',item:materialName,message:`Duplikat nama Material pada ${MATERIAL_SHEET}: ${materialName}. Import tetap diizinkan sebagai record terpisah untuk Review Pasca Import.`})
    const parsed={docId:materialDocId(materialName,occurrence),materialName,activeIngredient,unit,category,active:true};materials.push(parsed)
  ...
  const activities:ParsedActivity[]=activityBases.map(base=>{
    ...
    const docId=activityDocId(scope,base.description,base.occurrence)
    return{docId,activityCode:codeFor(docId),description:base.description,activity:base.activity,type:base.type,activityCategory:base.activityCategory,companyScope:scope,defaultUnitName:base.defaultUnitName,defaultShift:base.defaultShift,defaultForeman:base.defaultForeman,active,components,componentCount:components.length}
  })

  for(const key of compositionNames)if(!knownDescriptions.has(key))issues.push({level:'WARNING',item:key,message:`Komposisi "${key}" tidak memiliki Activity di ${VALIDATION_SHEET} dan tidak akan diimport.`})
  return{fileName,activities,materials,issues}
}

function activityChanges(previous:Record<string,unknown>,row:ParsedActivity){const changes:string[]=[];if(!sameText(previous.activityCode,row.activityCode))changes.push('code');if(!sameText(previous.description,row.description))changes.push('deskripsi');if(!sameText(previous.activity,row.activity))changes.push('activity');if(!sameText(previous.type,row.type))changes.push('type');if(!sameText(previous.activityCategory,row.activityCategory))changes.push('category');if(!sameText(previous.companyScope,row.companyScope))changes.push('scope');if(!sameText(previous.defaultUnitName||previous.unitName||previous.defaultUnit,row.defaultUnitName))changes.push('default unit');if(!sameText(previous.defaultShift||previous.shift,row.defaultShift))changes.push('default shift');if(!sameText(previous.defaultForeman||previous.foreman||previous.mandor,row.defaultForeman))changes.push('default mandor');if(Boolean(previous.active)!==row.active)changes.push('status');if(!sameArray(previous.components,row.components))changes.push('komposisi bahan');return changes}
...
export default function MasterActivityImportPanel({user,onOpenManage}:Props){
  ...
  const[busy,setBusy]=useState(false),[message,setMessage]=useState(''),[previewKind,setPreviewKind]=useState<'ACTIVITY'|'MATERIAL'>('ACTIVITY'),[filter,setFilter]=useState<'ALL'|ImportStatus>('ALL'),[qualityFilter,setQualityFilter]=useState<'ALL'|'WARNING'|'ERROR'|'INFO'>('ALL'),[postImportReviewCount,setPostImportReviewCount]=useState(0)
  ...
  function resetFile(){setParsed(null);setActivityPreview([]);setMaterialPreview([]);setMessage('');setFilter('ALL');setPreviewKind('ACTIVITY');setQualityFilter('ALL');setPostImportReviewCount(0)}
  ...
  async function chooseFile(file:File|null){
    if(!file)return
    setBusy(true);setMessage('Membaca Master Activity…');setParsed(null);setActivityPreview([]);setMaterialPreview([]);setPostImportReviewCount(0)
    try{
  ...
  async function importToFirestore(){
    if(!parsed||errors>0)return
    setBusy(true);setMessage('Verifikasi sebelum import…');setPostImportReviewCount(0)
    try{
      ...
      if(!changedActivities.length&&!changedMaterials.length){const reviewCount=await countPostImportReviewIssues();setPostImportReviewCount(reviewCount);setMessage(`${comparisonMessage('Tidak ada perubahan yang perlu diimport.',compared.activities,compared.materials)}\nFirestore tidak diubah.${reviewCount?`\nReview Pasca Import: ditemukan ${reviewCount} issue duplikat/kemiripan yang dapat diedit, dinonaktifkan, atau dihapus dari Kelola Master.`:'\nReview Pasca Import: tidak ditemukan issue duplikat.'}`);return}
      const confirmation=window.prompt(`Scope Activity: ${scope}\n\nActivity: ${changedActivities.length} ditulis (${changedActivities.filter(item=>item.status==='CREATE').length} baru, ${changedActivities.filter(item=>item.status==='UPDATE').length} update).\nMaterial: ${changedMaterials.length} ditulis (${changedMaterials.filter(item=>item.status==='CREATE').length} baru, ${changedMaterials.filter(item=>item.status==='UPDATE').length} update).\n${activityCounts.inactive} Activity tanpa komposisi akan disimpan INACTIVE.\nDuplikat nama/deskripsi TIDAK memblokir import; akan masuk Review Pasca Import.\nData yang hilang dari file TIDAK akan dihapus.\n\nKetik IMPORT untuk melanjutkan.`)
      ...
      const remainingActivity=after.activities.filter(item=>item.status!=='NO_CHANGE').length,remainingMaterial=after.materials.filter(item=>item.status!=='NO_CHANGE').length,reviewCount=await countPostImportReviewIssues();setPostImportReviewCount(reviewCount)
      ...
      const reviewText=reviewCount?`Review Pasca Import: ditemukan ${reviewCount} issue duplikat/kemiripan. Data tetap tersimpan; gunakan Kelola Master untuk Edit, Paksa INACTIVE, atau Hapus.`:'Review Pasca Import: tidak ditemukan issue duplikat.'
  ...
  return <section>
    ...
    <div className="panel form-stack">
      ...
      <p className="muted">File wajib berisi sheet <strong>List Validasi</strong>, <strong>Master Activity</strong>, dan <strong>Bahan</strong>. Kolom opsional <strong>Unit</strong>, <strong>Shift</strong>, dan <strong>Mandor/Foreman</strong> pada List Validasi akan menjadi resource default untuk Daily, Actual, dan QC. Activity tanpa komposisi bahan/dosis otomatis menjadi INACTIVE.</p>
      <div className="form-grid">
        ...
        <div><strong>Aturan Status</strong><p className="muted" style={{margin:'7px 0 0'}}>Ada komposisi + dosis valid → ACTIVE. Belum ada komposisi → INACTIVE. Data duplikat tetap dapat diimport untuk direview.</p></div>
      </div>
      ...
      <div className="row-actions"><button type="button" disabled={busy||!parsed||errors>0} onClick={()=>void refreshComparison()}>Bandingkan Firestore</button><button type="button" className="primary" disabled={busy||!parsed||errors>0||(!activityPreview.length&&!materialPreview.length)} onClick={()=>void importToFirestore()}>{busy?'Memproses…':'Confirm Import Firestore'}</button>{postImportReviewCount>0&&onOpenManage&&<button type="button" onClick={onOpenManage}>Review Pasca Import ({postImportReviewCount})</button>}</div>
    ...
    {parsed&&<>
      ...
      <div className="panel" ref={previewRef}>
        <div className="section-head"><div><h3>Preview Perubahan</h3><p className="muted">Bandingkan Activity dan Material sebelum import.</p></div><div className="row-actions premium-inline-filters"><select value={previewKind} onChange={e=>setPreviewKind(e.target.value as 'ACTIVITY'|'MATERIAL')}><option value="ACTIVITY">Activity</option><option value="MATERIAL">Material</option></select><select value={filter} onChange={e=>setFilter(e.target.value as 'ALL'|ImportStatus)}><option value="ALL">Semua</option><option value="CREATE">CREATE</option><option value="UPDATE">UPDATE</option><option value="NO_CHANGE">NO CHANGE</option></select></div></div>
        {previewKind==='ACTIVITY'?<div className="table-wrap"><table><thead><tr><th>Status</th><th>Code</th><th>Deskripsi</th><th>Activity</th><th>Type</th><th>Category</th><th>Scope</th><th>Default Resource</th><th>Active</th><th>Komposisi</th><th>Perubahan</th></tr></thead><tbody>{visibleActivities.map(row=><tr key={row.docId}><td><span className="badge">{row.status}</span></td><td>{row.activityCode}</td><td><strong>{row.description}</strong></td><td>{row.activity}</td><td>{row.type}</td><td>{row.activityCategory}</td><td>{row.companyScope}</td><td><div>{row.defaultUnitName||'-'}</div><div className="muted">Shift {row.defaultShift||'-'} · {row.defaultForeman||'-'}</div></td><td>{row.active?'ACTIVE':'INACTIVE'}</td><td>{row.components.length?row.components.map(component=><div key={`${row.docId}-${component.sequence}`}>{component.sequence}. {component.activeIngredient} — {new Intl.NumberFormat('id-ID',{maximumFractionDigits:4}).format(component.dosePerHa)} {component.unit}/Ha</div>):'-'}</td><td>{row.changes.join(', ')||'-'}</td></tr>)}{!visibleActivities.length&&<tr><td colSpan={11} className="empty">Tidak ada Activity pada filter ini.</td></tr>}</tbody></table></div>:<div className="table-wrap"><table><thead><tr><th>Status</th><th>Material</th><th>Bahan Aktif</th><th>Satuan</th><th>Kategori</th><th>Perubahan</th></tr></thead><tbody>{visibleMaterials.map(row=><tr key={row.docId}><td><span className="badge">{row.status}</span></td><td><strong>{row.materialName}</strong></td><td>{row.activeIngredient}</td><td>{row.unit}</td><td>{row.category}</td><td>{row.changes.join(', ')||'-'}</td></tr>)}{!visibleMaterials.length&&<tr><td colSpan={6} className="empty">Tidak ada Material pada filter ini.</td></tr>}</tbody></table></div>}
      </div>
      <div className="panel" ref={qualityRef}><div className="section-head"><div><h3>Data Quality</h3><p className="muted">ERROR hanya untuk data struktur yang tidak dapat diproses dan memblokir import. WARNING termasuk duplikat dan tetap dapat diimport untuk diperbaiki sesudahnya.</p></div><select className="premium-inline-filter" value={qualityFilter} onChange={e=>setQualityFilter(e.target.value as 'ALL'|'WARNING'|'ERROR'|'INFO')}><option value="ALL">Semua</option><option value="WARNING">WARNING</option><option value="ERROR">ERROR</option><option value="INFO">INFO</option></select></div><div className="table-wrap"><table><thead><tr><th>Level</th><th>Item</th><th>Catatan</th></tr></thead><tbody>{visibleIssues.map((issue,index)=><tr key={`${issue.item||'file'}-${index}`}><td>{issue.level}</td><td>{issue.item||'-'}</td><td>{issue.message}</td></tr>)}{!visibleIssues.length&&<tr><td colSpan={3} className="empty">Tidak ada catatan pada filter ini.</td></tr>}</tbody></table></div></div>
    </>}
  </section>
}
