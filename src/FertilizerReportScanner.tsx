...
export default function FertilizerReportScanner({master,defaults,onApply,onClose}:Props){
  ...
  async function run(){if(!file)return;setBusy(true);setError('');setProgress(8);setEngine('');setStage('Gemini AI membaca laporan...');try{const parsed=await scanFertilizerReportWithGemini(file,master,defaults);setRawText(parsed.rawText);setResult(parsed);setEngine('gemini');setProgress(100);setStage('Gemini AI selesai - periksa hasil sebelum diterapkan')}catch(aiError){const aiMessage=aiError instanceof Error?aiError.message:'Gemini AI gagal';setError('Gemini AI tidak tersedia: '+aiMessage+'. Sistem menggunakan OCR lokal sebagai fallback.');try{await runLocalOcr(file)}catch(localError){setError('Gemini AI gagal ('+aiMessage+') dan OCR lokal juga gagal: '+(localError instanceof Error?localError.message:'error tidak dikenal'));setStage('Scan gagal')}}finally{setBusy(false)}}
