...
export type ScanFilling={pengisianKe:number;dosis:string;statusHose:string;jenisPupuk:string;jumlah:string;hasilKerja:string;pemerataanPupuk:string}
export type ScanUnit={unit:string;noUnit:string;paddock:string;type:string;activity:string;catatan:string;fillings:ScanFilling[]}
export type FertilizerScanResult={date:string;shift:string;mandor:string;assistant:string;units:ScanUnit[];rawText:string;confidence:number;warnings:string[]}
...
export default function FertilizerReportScanner({master,defaults,onApply,onClose}:Props){
  const[file,setFile]=useState<File|null>(null),[preview,setPreview]=useState(''),[rawText,setRawText]=useState(''),[result,setResult]=useState<FertilizerScanResult|null>(null),[busy,setBusy]=useState(false),[progress,setProgress]=useState(0),[stage,setStage]=useState('Pilih foto laporan'),[error,setError]=useState('')
  ...
  function choose(next:File|null){if(!next)return;if(!next.type.startsWith('image/')){setError('Pilih file gambar laporan.');return}if(next.size>12_000_000){setError('Foto laporan maksimal 12 MB.');return}if(preview)URL.revokeObjectURL(preview);setFile(next);setPreview(URL.createObjectURL(next));setRawText('');setResult(null);setError('');setStage('Foto siap diproses');setProgress(0)}
  async function run(){if(!file)return;setBusy(true);setError('');setProgress(1);setStage('Menyiapkan OCR...');let worker:Awaited<ReturnType<(typeof import('tesseract.js'))['createWorker']>>|null=null;try{const source=await preprocess(file),mod=await import('tesseract.js');worker=await mod.createWorker(['eng','ind'],mod.OEM.LSTM_ONLY,{logger:m=>{if(typeof m.progress==='number')setProgress(Math.round(m.progress*100));if(m.status)setStage(String(m.status).replaceAll('_',' '))}});await worker.setParameters({preserve_interword_spaces:'1'});const recognized=await worker.recognize(source),txt=String(recognized.data.text||'').trim(),parsed=parseFertilizerReportText(txt,master,defaults,Number(recognized.data.confidence||0));setRawText(txt);setResult(parsed);setStage('OCR selesai - periksa hasil sebelum diterapkan');setProgress(100)}catch(e){setError(e instanceof Error?e.message:'OCR gagal memproses foto.');setStage('OCR gagal')}finally{if(worker)await worker.terminate().catch(()=>{});setBusy(false)}}
  ...
  return <div className="scan-backdrop" role="dialog" aria-modal="true"><div className="scan-dialog">
    <div className="scan-toolbar"><div><div className="eyebrow">FERTILIZER REPORT OCR</div><h3>Scan Laporan ke Unit Card</h3><p>OCR berjalan di browser. Selalu periksa hasil sebelum diterapkan.</p></div><button type="button" onClick={onClose} disabled={busy}>Tutup</button></div>
    <div className="scan-source-grid"><div className="scan-image-panel">{preview?<img src={preview} alt="Preview laporan"/>:<div className="scan-empty">Belum ada foto laporan.</div>}<input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={e=>choose(e.target.files?.[0]||null)}/><input ref={galleryRef} hidden type="file" accept="image/*" onChange={e=>choose(e.target.files?.[0]||null)}/><div className="row-actions"><button type="button" onClick={()=>cameraRef.current?.click()} disabled={busy}>Kamera</button><button type="button" onClick={()=>galleryRef.current?.click()} disabled={busy}>Galeri</button><button type="button" className="primary" onClick={()=>void run()} disabled={busy||!file}>{busy?'Memindai...':'Scan Laporan'}</button></div>{file&&<small>{file.name} - {(file.size/1024/1024).toFixed(1)} MB</small>}</div><div className="scan-progress-panel"><strong>{stage}</strong><div className="scan-progress"><span style={{width:String(progress)+'%'}}/></div><small>{progress}%</small>{error&&<div className="alert scan-error">{error}</div>}{result&&<div className="scan-confidence"><span>Confidence OCR</span><strong>{result.confidence}%</strong><span>Field perlu review</span><strong>{missing}</strong></div>}</div></div>
