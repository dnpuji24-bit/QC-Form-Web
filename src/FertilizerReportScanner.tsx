...
import { paddleOcrConfigured, scanWithPaddleOcr } from './paddleOcrClient'
...
export type FertilizerScanResult={date:string;shift:string;mandor:string;assistant:string;units:ScanUnit[];rawText:string;confidence:number;warnings:string[];sourceModel?:string}
...
export default function FertilizerReportScanner({master,defaults,onApply,onClose}:Props){
  ...
  function choose(next:File|null){if(!next)return;if(!next.type.startsWith('image/')){setError('Pilih file gambar laporan.');return}if(next.size>12_000_000){setError('Foto laporan maksimal 12 MB.');return}if(preview)URL.revokeObjectURL(preview);setFile(next);setPreview(URL.createObjectURL(next));setRawText('');setResult(null);setError('');setStage('Foto siap diproses');setProgress(0);setEngine('')}
  async function runLocalOcr(sourceFile=file){if(!sourceFile)return;let worker:Awaited<ReturnType<(typeof import('tesseract.js'))['createWorker']>>|null=null;setProgress(1);setStage('Fallback OCR lokal...');try{const source=await preprocess(sourceFile),mod=await import('tesseract.js');worker=await mod.createWorker(['eng','ind'],mod.OEM.LSTM_ONLY,{logger:m=>{if(typeof m.progress==='number')setProgress(Math.round(m.progress*100));if(m.status)setStage('OCR lokal: '+String(m.status).replaceAll('_',' '))}});await worker.setParameters({preserve_interword_spaces:'1'});const recognized=await worker.recognize(source),txt=String(recognized.data.text||'').trim(),parsed=parseFertilizerReportText(txt,master,defaults,Number(recognized.data.confidence||0));setRawText(txt);setResult(parsed);setEngine('tesseract');setStage('OCR lokal selesai - periksa hasil');setProgress(100)}finally{if(worker)await worker.terminate().catch(()=>{})}}
  async function runGemini(sourceFile:File){setProgress(42);setStage('Gemini AI membaca dan melengkapi laporan...');const parsed=await scanFertilizerReportWithGemini(sourceFile,master,defaults);setRawText(parsed.rawText);setResult(parsed);setEngine('gemini');setProgress(100);setStage('Gemini AI selesai - periksa hasil sebelum diterapkan');setError('')}
  async function run(){
    if(!file)return
    setBusy(true);setError('');setProgress(5);setEngine('')
    try{
      if(paddleOcrConfigured){
        try{
          setStage('PaddleOCR membaca laporan...');setProgress(18)
          const paddle=await scanWithPaddleOcr(file)
          const parsed=parseFertilizerReportText(paddle.text,master,defaults,paddle.confidence)
          parsed.sourceModel=paddle.ocrVersion||'PaddleOCR'
          const acceptable=paddle.confidence>=72&&parsed.warnings.length<=2
          if(acceptable){
            setRawText(paddle.text);setResult(parsed);setEngine('paddle');setProgress(100);setStage('PaddleOCR selesai - periksa hasil sebelum diterapkan');return
          }
          console.info('PaddleOCR terbaca tetapi perlu bantuan Gemini.',{confidence:paddle.confidence,warnings:parsed.warnings.length})
          setStage('PaddleOCR perlu dilengkapi - beralih ke Gemini...')
        }catch(paddleError){
          console.info('PaddleOCR tidak tersedia; beralih ke Gemini.',paddleError)
          setStage('PaddleOCR tidak tersedia - beralih ke Gemini...')
        }
      }
      try{await runGemini(file)}
      catch(aiError){
        const aiMessage=aiError instanceof Error?aiError.message:'Gemini AI gagal'
        console.warn('Gemini scan gagal; memakai OCR lokal.',aiError)
        const friendly=/high demand|\b500\b|\b503\b|\b429\b|temporar|UNAVAILABLE|RESOURCE_EXHAUSTED/i.test(aiMessage)?'Gemini sedang sibuk. Hasil sementara menggunakan OCR Lokal.':'Gemini belum dapat digunakan. Hasil sementara menggunakan OCR Lokal.'
        setError(friendly)
        try{await runLocalOcr(file)}catch(localError){setError(friendly+' OCR Lokal juga gagal: '+(localError instanceof Error?localError.message:'error tidak dikenal'));setStage('Scan gagal')}
      }
  ...
  return <div className="scan-backdrop" role="dialog" aria-modal="true"><div className="scan-dialog">
    <div className="scan-toolbar"><div><div className="eyebrow">FERTILIZER REPORT AI</div><h3>Scan Laporan ke Unit Card</h3><p>PaddleOCR mencoba lebih dulu untuk kecepatan. Jika hasil kurang lengkap, quota/cost guard aktif, timeout, atau service tidak tersedia, sistem otomatis memakai Gemini AI. OCR lokal menjadi fallback terakhir.</p></div><button type="button" onClick={onClose} disabled={busy}>Tutup</button></div>
    <div className="scan-source-grid"><div className="scan-image-panel">{preview?<img src={preview} alt="Preview laporan"/>:<div className="scan-empty">Belum ada foto laporan.</div>}<input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={e=>choose(e.target.files?.[0]||null)}/><input ref={galleryRef} hidden type="file" accept="image/*" onChange={e=>choose(e.target.files?.[0]||null)}/><div className="row-actions"><button type="button" onClick={()=>cameraRef.current?.click()} disabled={busy}>Kamera</button><button type="button" onClick={()=>galleryRef.current?.click()} disabled={busy}>Galeri</button><button type="button" className="primary" onClick={()=>void run()} disabled={busy||!file}>{busy?'Memindai...':'Scan Otomatis'}</button><button type="button" onClick={()=>{if(!file)return;setBusy(true);setError('');void runLocalOcr(file).catch(e=>setError(e instanceof Error?e.message:'OCR lokal gagal')).finally(()=>setBusy(false))}} disabled={busy||!file}>OCR Lokal</button></div>{file&&<small>{file.name} - {(file.size/1024/1024).toFixed(1)} MB</small>}</div><div className="scan-progress-panel"><strong>{stage}</strong><div className="scan-progress"><span style={{width:String(progress)+'%'}}/></div><small>{progress}%</small>{error&&<div className="alert scan-error">{error}</div>}{result&&<div className="scan-confidence"><span>Mesin pembaca</span><strong>{engine==='paddle'?`PaddleOCR${result?.sourceModel?' · '+result.sourceModel:''}`:engine==='gemini'?`Gemini AI${result?.sourceModel?' · '+result.sourceModel:''}`:engine==='tesseract'?'OCR Lokal':'-'}</strong><span>{engine==='gemini'?'Kelengkapan hasil':'Confidence OCR'}</span><strong>{result.confidence}%</strong><span>Field perlu review</span><strong>{missing}</strong></div>}</div></div>
    ...
    <div className="scan-actions"><div><strong>Belum disimpan.</strong><span>Hasil scan hanya mengisi form dan masih dapat dikoreksi.</span></div><div className="row-actions"><button type="button" onClick={()=>file&&onApply(result,'append',file)}>Tambahkan ke Form</button><button type="button" className="primary" onClick={()=>file&&onApply(result,'replace',file)}>Ganti Form dengan Hasil Scan</button></div></div></div>}
  </div></div>
}
