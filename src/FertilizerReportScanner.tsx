...
import { applyFertilizerOcrMemory, saveFertilizerOcrFeedback } from './fertilizerOcrLearning'
...
export default function FertilizerReportScanner({user,master,defaults,onApply,onClose}:Props){
  ...
  const cameraRef=useRef<HTMLInputElement>(null),galleryRef=useRef<HTMLInputElement>(null),originalAiRef=useRef<FertilizerScanResult|null>(null),originalOcrRef=useRef<FertilizerScanResult|null>(null)
  ...
  async function applyReviewed(mode:ScanApplyMode){
    ...
    let learningFeedbackSaved=false,learningChangedFields=0,learningEngine:'gemini'|'ocr'|undefined
    if(engine==='gemini'&&originalAiRef.current){
      setBusy(true);setStage('Menyimpan Gemini Correction Learning...')
      try{
        ...
        learningFeedbackSaved=feedback.saved;learningChangedFields=feedback.changedFieldCount;learningEngine='gemini'
      }finally{setBusy(false)}
    }else if(engine==='tesseract'&&originalOcrRef.current){
      setBusy(true);setStage('Menyimpan OCR Correction Learning...')
      try{
        const feedback=await saveFertilizerOcrFeedback({user,original:originalOcrRef.current,corrected:result,sourceFileName:file.name,applyMode:mode})
        learningFeedbackSaved=feedback.saved;learningChangedFields=feedback.changedFieldCount;learningEngine='ocr'
      }finally{setBusy(false)}
    }
    onApply({...result,learningFeedbackSaved,learningChangedFields,learningEngine},mode,file)
  }
  ...
  return <div className="scan-backdrop" role="dialog" aria-modal="true"><div className="scan-dialog">
    ...
    <div className="scan-actions"><div><strong>Belum disimpan.</strong><span>{engine==='gemini'?'Koreksi akan masuk Gemini Correction Learning saat hasil diterapkan.':engine==='tesseract'?'Koreksi akan masuk OCR Correction Learning; pola berulang akan digunakan pada scan berikutnya.':'Hasil scan hanya mengisi form dan masih dapat dikoreksi.'}</span></div><div className="row-actions"><button type="button" disabled={busy} onClick={()=>void applyReviewed('append')}>Tambahkan ke Form</button><button type="button" className="primary" disabled={busy} onClick={()=>void applyReviewed('replace')}>{busy?'Menyimpan Learning...':'Ganti Form dengan Hasil Scan'}</button></div></div></div>}
  </div></div>
}
