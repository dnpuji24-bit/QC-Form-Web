...
export default function ActualPlanListPanel({user,selectedDate,compact=false,refreshKey=0,onEditActual,onDuplicateActual,onChanged}:Props){
  ...
  return <section><div className="section-head"><div><div className="eyebrow">ACTUAL PLAN</div><h2>Daftar Actual / Daily Report</h2><p className="muted">Edit memakai form utama Actual agar perubahan Daily/PID mengganti record yang sama dan Actual ID tetap.</p></div><button type="button" disabled={busy} onClick={()=>void load()}>{busy?'Memuat…':'Refresh'}</button></div>{message&&<div className="alert">{message}</div>}
    <div className="panel premium-filter-panel compact-filter-panel" style={{marginTop:18}}><div className="premium-filter-grid filter-grid-auto"><label className="premium-filter-field"><span>Bulan</span><select value={month} onChange={e=>setMonth(e.target.value)}><option value="ALL">Semua Bulan</option>{months.map(x=><option key={x}>{x}</option>)}</select></label><label className="premium-filter-field"><span>Sumber</span><select value={source} onChange={e=>setSource(e.target.value)}><option value="ALL">Semua Sumber</option><option value="MONTHLY">MONTHLY</option><option value="ADHOC">ADHOC</option><option value="SUPPORT">SUPPORT</option><option value="HISTORICAL">HISTORICAL</option></select></label><label className="premium-filter-field"><span>Daily Link</span><select value={link} onChange={e=>setLink(e.target.value)}><option value="ALL">Semua Status</option><option value="LINKED">LINKED</option><option value="NOT_FOUND">PENDING</option><option value="AMBIGUOUS">AMBIGUOUS</option></select></label><label className="premium-filter-field"><span>Company</span><select value={company} onChange={e=>{setCompany(e.target.value);setFarm('ALL')}}><option value="ALL">Semua Company</option>{companies.map(x=><option key={x}>{x}</option>)}</select></label><label className="premium-filter-field"><span>Farm</span><select value={farm} onChange={e=>setFarm(e.target.value)}><option value="ALL">Semua Farm</option>{farms.map(x=><option key={x}>{x}</option>)}</select></label><label className="premium-filter-field"><span>Kegiatan</span><select value={activity} onChange={e=>setActivity(e.target.value)}><option value="ALL">Semua Kegiatan</option>{activities.map(x=><option key={x}>{x}</option>)}</select></label><label className="premium-filter-field"><span>Cari</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Actual ID / Daily ID / Monthly ID / PID / mandor"/></label></div></div>
