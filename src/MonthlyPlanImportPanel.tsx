...
const REQUIRED_HEADERS=['Tahun','Bulan','Input Date','Start Date','End Date','Weekly','Plan ID Baru','Deskripsi','Paddock','Luas Target(Ha)','Variety','Activity','Keterangan']
...
function parseWorkbook(buffer:ArrayBuffer,fileName:string,refs:MasterRefs):ParsedFile{
  ...
  rows.forEach((row,index)=>{
    ...
    const monthKey=`${year}-${pad(monthNumber)}`,inputDate=dateFromValue(rowValue(row,'Input Date'),year,monthNumber),startDate=dateFromValue(rowValue(row,'Start Date'),year,monthNumber),endDate=dateFromValue(rowValue(row,'End Date'),year,monthNumber),calculatedBalanceHa=0
...
export default function MonthlyPlanImportPanel({user}:Props){
  ...
  return <section>
    <div className="section-head"><div><div className="eyebrow">MONTHLY PLAN</div><h2>Update / Import Excel</h2><p className="muted">Gunakan format Monthly Plan final. Plan ID Baru menjadi identitas record. Kolom Status, Luas Dikerjakan, dan Balance pada Excel tidak dipakai sebagai sumber progress; ketiganya dihitung dari Actual Plan yang linked. Data yang pernah diedit melalui Web tetap ditandai PROTECTED.</p></div></div>
    <div className="panel">
    ...
    {parsed&&<>
      ...
      <div className="panel" style={{marginTop:18}}><div className="section-head"><div><h3>Preview Import</h3><p className="muted">PROTECTED = record pernah diubah dari Web dan tidak akan ditimpa otomatis.</p></div></div><div className="table-wrap"><table><thead><tr><th>Status</th><th>Plan ID Baru</th><th>Bulan/Week</th><th>Company</th><th>PID</th><th>Deskripsi</th><th>Target</th><th>Luas Dikerjakan</th><th>Balance</th><th>Perubahan</th></tr></thead><tbody>{preview.slice(0,250).map(row=><tr key={row.docId}><td><strong>{row.importStatus}</strong></td><td>{row.planLineId}</td><td>{row.monthKey} / {row.week}</td><td>{row.companyCode||'-'}</td><td>{row.pid}</td><td>{row.description}</td><td>{formatHa(row.targetAreaHa)}</td><td><span className="muted">Dari Actual Plan</span></td><td><span className="muted">Dari Actual Plan</span></td><td>{row.changes.join(', ')||'-'}</td></tr>)}</tbody></table></div>{preview.length>250&&<p className="muted">Menampilkan 250 dari {preview.length} record.</p>}</div>
    </>}
  </section>
}
