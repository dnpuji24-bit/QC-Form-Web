...
type ActivityRow={id:string;activityCode:string;description:string;activity:string;type:string;activityCategory:string;companyScope:string;defaultUnitName:string;defaultShift:string;defaultForeman:string;active:boolean;components:ActivityComponent[]}
...
type ValidationForm={id:string;activityCode:string;description:string;activity:string;type:string;activityCategory:string;companyScope:string;defaultUnitName:string;defaultShift:string;defaultForeman:string}
...
const emptyValidation:ValidationForm={id:'',activityCode:'',description:'',activity:'',type:'SPRAY',activityCategory:'HERBICIDE',companyScope:'GLOBAL',defaultUnitName:'',defaultShift:'',defaultForeman:''}
...
function activityFromData(id:string,data:Record<string,unknown>):ActivityRow{return{id,activityCode:text(data.activityCode),description:text(data.description),activity:text(data.activity),type:text(data.type).toUpperCase(),activityCategory:text(data.activityCategory).toUpperCase(),companyScope:text(data.companyScope)||'GLOBAL',defaultUnitName:text(data.defaultUnitName||data.unitName||data.defaultUnit),defaultShift:text(data.defaultShift||data.shift),defaultForeman:text(data.defaultForeman||data.foreman||data.mandor),active:data.active===true,components:componentFromData(data.components)}}
...
export default function MasterActivityManagePanel({user}:Props){
  ...
  function editValidation(row:ActivityRow){setValidationForm({id:row.id,activityCode:row.activityCode,description:row.description,activity:row.activity,type:row.type,activityCategory:row.activityCategory,companyScope:row.companyScope,defaultUnitName:row.defaultUnitName,defaultShift:row.defaultShift,defaultForeman:row.defaultForeman});setView('VALIDATION')}
  ...
  async function saveValidation(){
    const description=validationForm.description.trim(),activity=validationForm.activity.trim(),type=validationForm.type.trim().toUpperCase(),activityCategory=validationForm.activityCategory.trim().toUpperCase(),companyScope=validationForm.companyScope.trim().toUpperCase()||'GLOBAL',defaultUnitName=validationForm.defaultUnitName.trim(),defaultShift=validationForm.defaultShift.trim(),defaultForeman=validationForm.defaultForeman.trim()
