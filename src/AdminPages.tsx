...
export function UsersApproval({ token }: UsersProps) {
  ...
  async function load() {
    ...
    try {
      const result = await qcApi.users(token)
      const list = (result.users || result.data || []) as User[]
      setUsers(list)
      setRoleDraft(Object.fromEntries(list.map((u) => [u.username, u.role])))
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Gagal memuat pengguna') }
