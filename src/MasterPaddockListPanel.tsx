...
export default function MasterPaddockListPanel(){
  ...
  const filtered=useMemo(()=>{
    ...
    return rows.filter(row=>(company==='ALL'||row.companyCode===company)&&(farm==='ALL'||row.farm===farm)&&(stage==='ALL'||row.currentStage===stage)&&(!needle||`${row.pid} ${row.companyCode} ${row.farm} ${row.block} ${row.paddock} ${row.variety}`.toLowerCase().includes(needle)))
  },[rows,company,farm,stage,query])
  ...
  const farmSummary=useMemo(()=>{
    const map=new Map<string,{company:string;farm:string;pids:number;area:number;pc:number;r1:number;r2:number}>()
    filtered.forEach(row=>{const key=`${row.companyCode}__${row.farm}`,item=map.get(key)||{company:row.companyCode,farm:row.farm,pids:0,area:0,pc:0,r1:0,r2:0};item.pids+=1;item.area+=row.areaPlantedHa;if(row.currentStage==='PC')item.pc+=1;if(row.currentStage==='R1')item.r1+=1;if(row.currentStage==='R2')item.r2+=1;map.set(key,item)})
  ...
  return <section>
    <div className="section-head"><div><div className="eyebrow">FIRESTORE MASTER</div><h2>Daftar Paddock</h2><p className="muted">Rekapan Master Paddock yang sudah benar-benar tersimpan di Firestore. Bukan preview file Excel.</p></div><button type="button" disabled={busy} onClick={()=>void load()}>{busy?'Memuat…':'Refresh'}</button></div>
    {message&&<div className="alert">{message}</div>}
    <div className="panel">
      <div className="record-filters">
        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari PID / variety / block…"/>
        ...
        <button type="button" onClick={()=>{setQuery('');setCompany('ALL');setFarm('ALL');setStage('ALL')}}>Reset</button>
      </div>
    </div>
    <div className="stats-grid">
      <div className="stat"><span>PID</span><strong>{filtered.length}</strong></div>
      <div className="stat"><span>Area Plan</span><strong>{totalArea.toFixed(2)} Ha</strong></div>
    ...
    <div className="panel">
      <div className="section-head"><div><h3>Rekap per Farm</h3><p className="muted">Mengikuti filter Company, Farm, Stage, dan pencarian di atas.</p></div></div>
      <div className="table-wrap"><table><thead><tr><th>Company</th><th>Farm</th><th>PID</th><th>Area Plan</th><th>PC</th><th>R1</th><th>R2</th></tr></thead><tbody>{farmSummary.map(item=><tr key={`${item.company}-${item.farm}`}><td>{item.company}</td><td>{item.farm||'-'}</td><td>{item.pids}</td><td>{item.area.toFixed(4)} Ha</td><td>{item.pc}</td><td>{item.r1}</td><td>{item.r2}</td></tr>)}{!farmSummary.length&&<tr><td colSpan={7} className="empty">Belum ada Master Paddock sesuai filter.</td></tr>}</tbody></table></div>
    </div>
    <div className="panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>Company</th><th>Farm</th><th>PID</th><th>Block</th><th>Paddock</th><th>Variety</th><th>Area Plan</th><th>Stage</th><th>Harvest Stage</th><th>Sisa Stage</th><th>Last Harvest</th><th>Source</th></tr></thead><tbody>{filtered.map(row=><tr key={row.pid}><td>{row.companyCode}</td><td>{row.farm||'-'}</td><td><strong>{row.pid}</strong></td><td>{row.block||'-'}</td><td>{row.paddock||'-'}</td><td>{row.variety||'-'}</td><td>{row.areaPlantedHa.toFixed(4)} Ha</td><td><span className="badge">{row.currentStage}</span></td><td>{row.harvestedCurrentStageHa.toFixed(4)} Ha</td><td>{row.remainingHarvestCurrentStageHa.toFixed(4)} Ha</td><td>{formatDate(row.lastHarvestDate)}</td><td>{row.sourceFileName||'-'}</td></tr>)}{!filtered.length&&<tr><td colSpan={12} className="empty">Belum ada Master Paddock tersimpan atau tidak ada data sesuai filter.</td></tr>}</tbody></table></div>
    </div>
    <div className="panel">
