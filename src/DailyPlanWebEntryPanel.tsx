...
type WorkDraft={id:string;sourceType:'MONTHLY'|'ADHOC'|'SUPPORT';search:string;monthlyId:string;adhocActivity:string;adhocPid:string;area:string;manpower:string;unitName:string;unitReady:string;unitStandby:string;unitBreakdown:string;notes:string}
...
function monthlyOptionLabel(row:Monthly){return row.planLineId+' — '+row.pid+' — '+(row.description||row.activity)}
...
export default function DailyPlanWebEntryPanel({user}:Props){
  ...
  const[draft,setDraft]=useState<Draft>(()=>({...initial,works:initial.works?.length?initial.works.map(w=>({...blankWork(),...w} as WorkDraft)):[blankWork()]})),[busy,setBusy]=useState(false),[message,setMessage]=useState('')
  ...
  function addWork(copy?:WorkDraft){setDraft(current=>({...current,works:[...current.works,{...(copy||blankWork()),id:planRowId('daily'),monthlyId:copy?.monthlyId||'',search:copy?.search||'',area:'',notes:''}]}))}
  ...
  function reset(){if(!window.confirm('Kosongkan draft input Daily Plan ini?'))return;clearPlanDraft(draftKey);setDraft({date:new Date().toISOString().slice(0,10),shift:'1',foreman:'',works:[blankWork()]});setMessage('Draft Daily Plan dikosongkan.')}
  ...
  async function save(e:FormEvent){e.preventDefault();if(!draft.date||!draft.shift){setMessage('Tanggal dan Shift wajib diisi.');return}
    const invalid=workInfo.find(x=>x.area<=0||(x.work.sourceType==='MONTHLY'&&!x.selected)||(x.work.sourceType!=='MONTHLY'&&(!x.work.adhocActivity.trim()||!x.work.adhocPid.trim())))
    if(invalid){setMessage(invalid.area<=0?'Luas Plan Harian harus lebih dari 0.':invalid.work.sourceType==='MONTHLY'?'Pilih Monthly Plan untuk pekerjaan MONTHLY.':'ADHOC/SUPPORT wajib memiliki Nama Kegiatan dan Paddock.');return}
    const over=workInfo.filter(x=>x.work.sourceType==='MONTHLY'&&x.selected&&x.area>x.remaining+0.0001)
    if(over.length&&!window.confirm(over.length+' pekerjaan melebihi sisa Monthly Plan. Tetap simpan?'))return
    const keys=new Set<string>(),batchDup=workInfo.filter(x=>{const k=x.work.sourceType==='MONTHLY'?(x.selected?.planLineId||''):[x.work.sourceType,x.work.adhocPid,x.work.adhocActivity].join('|');if(!k)return false;if(keys.has(k))return true;keys.add(k);return false})
    ...
    const existingDup=workInfo.filter(x=>x.work.sourceType==='MONTHLY'&&x.selected&&daily.some(d=>d.date===draft.date&&d.shift===draft.shift&&d.monthlyPlanLineId===x.selected?.planLineId))
    ...
    try{
      ...
      const created:Daily[]=[]
      for(const info of workInfo){seq++;const selected=info.selected,dailyPlanId=prefix+String(seq).padStart(4,'0'),isMonthly=info.work.sourceType==='MONTHLY',pid=isMonthly?(selected?.pid||''):info.work.adhocPid.toUpperCase(),paddock=info.paddock,activity=isMonthly?(selected?.activity||''):info.work.adhocActivity.trim(),description=isMonthly?(selected?.description||''):info.work.adhocActivity.trim(),components=isMonthly?(selected?.componentsSnapshot||[]):(info.manualActivity?.componentsSnapshot||[]);batch.set(doc(db,'daily_plans',dailyPlanId),{dailyPlanId,sourcePlanIdRaw:isMonthly?(selected?.planLineId||''):'',sourceType:info.work.sourceType,monthlyLinkStatus:isMonthly?'LINKED':'NOT_APPLICABLE',monthlyPlanLineId:isMonthly?(selected?.planLineId||''):'',date:draft.date,year:Number(draft.date.slice(0,4)),monthKey:draft.date.slice(0,7),shift:draft.shift,activity,description,paddockRaw:pid,pid,areaHa:info.area,areaUnit:'Ha',manpower:planNum(info.work.manpower),unitName:info.work.unitName,unitReady:planNum(info.work.unitReady),unitStandby:planNum(info.work.unitStandby),unitBreakdown:planNum(info.work.unitBreakdown),foreman:draft.foreman,notes:info.work.notes,companyCode:isMonthly?(selected?.companyCode||''):(paddock?.companyCode||''),farm:isMonthly?(selected?.farm||''):(paddock?.farm||''),stage:isMonthly?(selected?.stage||''):(paddock?.stage||''),masterVariety:isMonthly?(selected?.masterVariety||''):(paddock?.variety||''),masterPending:!isMonthly&&!paddock,materials:info.materials,componentsSnapshot:components,type:isMonthly?(selected?.type||''):'',activityCategory:isMonthly?(selected?.activityCategory||''):'',sourceOrigin:'WEB',lastModifiedSource:'WEB',createdAt:serverTimestamp(),createdBy:username,updatedAt:serverTimestamp(),updatedBy:username});created.push({dailyPlanId,monthlyPlanLineId:isMonthly?(selected?.planLineId||''):'',date:draft.date,shift:draft.shift,areaHa:info.area})}
    ...
    <form onSubmit={save} className="plan-entry-form">
      ...
      <section className="panel plan-section"><div className="plan-section-title"><div><span className="eyebrow">PEKERJAAN HARIAN</span><h3>Monthly → Daily</h3></div><button type="button" onClick={()=>addWork()}>+ Tambah Pekerjaan</button></div>
        <div className="plan-work-list">{workInfo.map((info,index)=><div className="plan-work-card" key={info.work.id}><div className="plan-work-head"><strong>Pekerjaan {index+1}</strong><div className="row-actions">{info.selected&&<button type="button" onClick={()=>addWork(info.work)}>Duplikat</button>}<button type="button" className="danger" onClick={()=>removeWork(info.work.id)}>Hapus</button></div></div><div className="plan-grid">
          ...
          {info.work.sourceType==='MONTHLY'?<><label className="plan-span-2"><span>Monthly Plan — ketik untuk mencari</span><input list={'monthly-plan-options-'+info.work.id} value={info.work.search} onFocus={e=>e.currentTarget.select()} onChange={e=>{const value=e.target.value,matched=monthly.find(row=>monthlyOptionLabel(row)===value||row.planLineId===value);patchWork(info.work.id,{search:value,monthlyId:matched?.id||''})}} placeholder="Ketik G-007 / pre / top dressing"/><datalist id={'monthly-plan-options-'+info.work.id}>{info.choices.map(x=><option key={x.id} value={monthlyOptionLabel(x)}/>)}</datalist><small className="plan-search-hint">{info.selected?'Terpilih: '+monthlyOptionLabel(info.selected):info.work.search?info.choices.length+' pilihan cocok — pilih salah satu suggestion':'Ketik paddock atau awal nama kegiatan'}</small></label></>:<><label><span>Nama Kegiatan</span><input list={'daily-activities-'+info.work.id} value={info.work.adhocActivity} onChange={e=>patchWork(info.work.id,{adhocActivity:e.target.value})} placeholder="Contoh: Repair Road"/><datalist id={'daily-activities-'+info.work.id}>{activities.map(a=><option key={a.id} value={a.activity||a.description}/>)}</datalist></label><label className="plan-span-2"><span>Paddock</span><input list={'daily-paddocks-'+info.work.id} value={info.work.adhocPid} onChange={e=>patchWork(info.work.id,{adhocPid:e.target.value.toUpperCase()})} placeholder="Contoh: JAGF-2-A-007"/><datalist id={'daily-paddocks-'+info.work.id}>{paddocks.map(p=><option key={p.pid} value={p.pid}/>)}</datalist></label></>}
        ...
        </div>{info.work.sourceType==='MONTHLY'&&info.selected&&<div className="plan-inline-info"><span>Monthly <strong>{info.selected.planLineId}</strong></span><span>PID <strong>{info.selected.pid}</strong></span><span>Kegiatan <strong>{info.selected.description||info.selected.activity}</strong></span><span>Target <strong>{planHa(info.selected.targetAreaHa)}</strong></span><span>Terjadwal <strong>{planHa(info.scheduled)}</strong></span><span>Sisa <strong className={info.area>info.remaining?'plan-danger-text':''}>{planHa(info.remaining)}</strong></span></div>}{info.work.sourceType!=='MONTHLY'&&<div className="plan-inline-info"><span>Sumber <strong>{info.work.sourceType}</strong></span><span>PID <strong>{info.work.adhocPid||'-'}</strong></span><span>Kegiatan <strong>{info.work.adhocActivity||'-'}</strong></span></div>}{info.materials.length>0&&<div className="plan-material-mini"><strong>Bahan & dosis yang digunakan</strong>{info.materials.map(m=><small key={m.material}>{m.material}: <b>{m.dosePerHa} {m.unit}/Ha</b> → Total {m.totalMaterial.toLocaleString('id-ID',{maximumFractionDigits:4})} {m.unit}</small>)}</div>}</div>)}</div>
      </section>
