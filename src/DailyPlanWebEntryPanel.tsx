...
export default function DailyPlanWebEntryPanel({user}:Props){
  ...
  const[state,setState]=useState<DraftState>(initial),[busy,setBusy]=useState(false),[message,setMessage]=useState('')
  ...
  function draftTransfers(infos:WorkInfo[]):DailyPlanTransfer[]{return infos.flatMap(group=>group.pids.map((row,index)=>{const selected=row.selected,isMonthly=group.work.sourceType==='MONTHLY',pid=isMonthly?(selected?.pid||''):row.pid.pid.toUpperCase(),activity=isMonthly?(selected?.activity||group.work.activitySearch):group.work.activitySearch;return{dailyPlanId:'draft-'+group.work.id+'-'+index,workGroupId:group.work.id,date:state.date,shift:group.work.shift,sourceType:group.work.sourceType,monthlyPlanLineId:isMonthly?(selected?.planLineId||''):'',companyCode:isMonthly?(selected?.companyCode||''):(row.paddock?.companyCode||''),farm:isMonthly?(selected?.farm||''):(row.paddock?.farm||''),pid,activity,description:isMonthly?(selected?.description||group.work.activitySearch):group.work.activitySearch,areaHa:row.area,manpower:planNum(group.work.manpower),unitName:group.work.unitName,unitReady:planNum(group.work.unitReady),unitStandby:planNum(group.work.unitStandby),unitBreakdown:planNum(group.work.unitBreakdown),foreman:group.work.foreman,notes:group.work.notes,materials:row.materials}}))}
