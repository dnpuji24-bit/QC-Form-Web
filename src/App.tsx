...
export default function App() {
  ...
  async function logout() {
    try {
      if (token) await qcApi.logout(token)
    } catch {
      // Session is cleared locally even if the network request fails.
    }
    clearSession()
  }

  if (!token || !user) {
    return <AuthScreen onLogin={saveSession} />
  }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="content">
        ...
        {view === 'dashboard' && <Dashboard records={records} loading={busy} onRefresh={() => void refreshRecords()} />}
...
function AuthScreen({ onLogin }: { onLogin: (token: string, user: User) => void }) {
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage('')
    try {
      const result = await qcApi.register(data)
      setMessage(result.message || 'Pendaftaran terkirim.')
      form.reset()
      setMode('login')
    } catch (error) {
  ...
  return (
    <main className="auth-page">
      <section className="auth-card">
        ...
        <div className="eyebrow">PT. GLOBAL PAPUA ABADI</div>
        <h1>QC Form Web</h1>
        ...
        {mode === 'login' ? (
          <form className="form-stack" onSubmit={(event) => void login(event)}>
            <label>Username atau email<input name="username" autoComplete="username" required /></label>
            <label>Kata sandi<input name="password" type="password" autoComplete="current-password" minLength={4} required /></label>
            <button className="primary" disabled={busy}>{busy ? 'Memeriksa…' : 'Masuk'}</button>
          </form>
        ) : (
          <form className="form-stack" onSubmit={(event) => void register(event)}>
            <label>Nama lengkap<input name="fullName" minLength={3} required /></label>
            <label>Username<input name="username" minLength={3} required /></label>
            <label>Email<input name="email" type="email" required /></label>
            <label>Role
              <select name="role" defaultValue="mandor_spraying">
                <option value="mandor_spraying">Mandor Spraying</option>
                <option value="mandor_fertilizer">Mandor Fertilizer</option>
                <option value="asisten">Asisten</option>
                <option value="pengunjung">Pengunjung</option>
              </select>
            </label>
            <label>Kata sandi<input name="password" type="password" minLength={8} required /></label>
            <button className="primary" disabled={busy}>{busy ? 'Mengirim…' : 'Kirim pendaftaran'}</button>
          </form>
        )}
      </section>
    </main>
  ...
  return (
    <section>
      <div className="section-head">
        <div><div className="eyebrow">RINGKASAN</div><h2>Dashboard QC</h2></div>
        <button className="secondary" onClick={onRefresh} disabled={loading}>{loading ? 'Memuat…' : 'Refresh'}</button>
      </div>
      <div className="stats-grid">
        <Stat label="Total record" value={records.length} />
        <Stat label="Spraying" value={summary.spray} />
        <Stat label="Fertilizer" value={summary.fertilizer} />
        <Stat label="Draft / sync" value={summary.draft} />
        <Stat label="Uploaded" value={summary.uploaded} />
      </div>
      <div className="panel">
        <h3>Migrasi aman</h3>
        <p>Dashboard ini membaca endpoint <code>records</code> yang sama seperti aplikasi lama. Tidak ada perubahan pada Spreadsheet, Record ID, role, atau API Apps Script.</p>
      </div>
    </section>
  )
}
...
function Records({ records, loading, onRefresh }: { records: QcRecord[]; loading: boolean; onRefresh: () => void }) {
  ...
  const filtered = useMemo(() => records.filter((record) => {
    ...
    const text = `${record.date} ${record.paddock} ${record.name || ''} ${record.status || ''}`.toLowerCase()
  ...
  return (
    <section>
      <div className="section-head">
        <div><div className="eyebrow">MONITORING</div><h2>Data QC</h2></div>
        <button className="secondary" onClick={onRefresh} disabled={loading}>{loading ? 'Memuat…' : 'Refresh'}</button>
      </div>
      <div className="filters">
        <input placeholder="Cari paddock, mandor, status…" value={query} onChange={(e) => setQuery(e.target.value)} />
        <select value={type} onChange={(e) => setType(e.target.value as typeof type)}>
          <option value="all">Semua jenis</option><option value="spray">Spraying</option><option value="fertilizer">Fertilizer</option>
        </select>
      </div>
      <div className="table-wrap">
        <table>
          <thead><tr><th>Tanggal</th><th>Jenis</th><th>Paddock</th><th>Mandor</th><th>Status</th><th>Simpan</th></tr></thead>
          <tbody>
            {filtered.map((record) => <tr key={record.id}><td>{record.date}</td><td>{record.formType}</td><td>{record.paddock}</td><td>{record.name || '-'}</td><td>{record.status || '-'}</td><td>{record.saveType || '-'}</td></tr>)}
            {!filtered.length && <tr><td colSpan={6} className="empty">Belum ada data yang cocok.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
...
function Settings() {
  ...
  function save(event: FormEvent) {
    ...
    try {
      setApiUrl(url.trim())
      setMessage('URL Apps Script tersimpan di perangkat ini.')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'URL tidak valid')
    }
  }
  return (
    <section>
      <div className="section-head"><div><div className="eyebrow">KONFIGURASI</div><h2>Pengaturan</h2></div></div>
      <form className="panel form-stack" onSubmit={save}>
        <label>Apps Script API URL<input value={url} onChange={(e) => setUrl(e.target.value)} /></label>
        <p className="muted">Backend tetap menggunakan deployment <code>/exec</code> yang sama.</p>
        {message && <div className="alert">{message}</div>}
