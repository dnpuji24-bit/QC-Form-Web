...
const EMPTY:FormState={code:'',name:'',prefixes:'',active:true}

function dataFromDoc(id:string,data:Record<string,unknown>):CompanyRecord{
  return{
    id,
    code:normalizeCompanyCode(String(data.code||id)),
...
export default function CompanyMasterPanel({user}:Props){
  ...
  function startEdit(company:CompanyRecord){
    ...
    setForm({code:company.code,name:company.name,prefixes:company.prefixes.join(', '),active:company.active})
  ...
  return <section>
    ...
    <div className="panel form-stack">
      ...
      <p className="muted">Mapping ini dikelola manual. Contoh: prefix <strong>JAGF</strong> → <strong>GPA</strong>. PID yang prefix-nya belum terdaftar akan ditolak pada import Master Paddock.</p>
    ...
    <div className="panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>Code</th><th>Perusahaan</th><th>Prefix PID</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
        {companies.map(company=><tr key={company.id}><td><strong>{company.code}</strong>{company.fallback&&<div className="muted">default sementara</div>}</td><td>{company.name}</td><td>{company.prefixes.join(', ')||'-'}{company.prefixes.some(prefix=>duplicatePrefixes.has(normalizePrefix(prefix)))&&<div className="muted">⚠ prefix duplikat</div>}</td><td>{company.active?'ACTIVE':'INACTIVE'}</td><td><button type="button" disabled={!canEdit||busy} onClick={()=>startEdit(company)}>Edit</button></td></tr>)}
        {!companies.length&&<tr><td colSpan={5} className="empty">Belum ada Master Company.</td></tr>}
      </tbody></table></div>
    </div>
  </section>
}
