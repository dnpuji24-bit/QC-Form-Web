...
export default function ActualPlanWebEntryPanel({user,prefillDailyPlanIds=[]}:Props){
  ...
  async function save(e:FormEvent){e.preventDefault();if(!draft.date){setMessage('Tanggal Actual wajib diisi.');return}
    ...
    try{
      ...
      await batch.commit();setActuals(rows=>[...rows,...created]);clearPlanDraft(draftKey);setDraft(current=>({...current,works:[blankWork()]}));setMessage(created.length+' Actual berhasil disimpan. Total '+planHa(totals.area)+'; progress Monthly akan mengikuti link Daily → Monthly.')
